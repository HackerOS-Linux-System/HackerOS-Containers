...
pub fn apply_seccomp(profile_path: &str) -> Result<()> {
    ...
    .map_err(|e| miette!("Failed to read seccomp profile '{}': {}", profile_path, e))?;
    let profile: SeccompProfile = serde_json::from_str(&content)
    .map_err(|e| miette!("Invalid seccomp profile: {}", e))?;

    let default_action = str_to_action(&profile.default_action);
    let mut filter = ScmpFilterContext::new_filter(default_action)
    .map_err(|e| miette!("Cannot create seccomp filter: {}", e))?;

    for arch in &profile.architectures {
        filter.add_arch(str_to_arch(arch))
        .map_err(|e| miette!("Cannot add arch {}: {}", arch, e))?;
    }

    for rule in &profile.syscalls {
        let action = str_to_action(&rule.action);
        for name in &rule.names {
            let syscall = ScmpSyscall::from_name(name)
            .map_err(|e| miette!("Unknown syscall '{}': {}", name, e))?;
            filter.add_rule(action, syscall)
            .map_err(|e| miette!("Cannot add rule for '{}': {}", name, e))?;
        }
    }

    filter.load().map_err(|e| miette!("Cannot load seccomp filter: {}", e))?;
    Ok(())
}
...
fn str_to_action(s: &str) -> ScmpAction {
    match s {
